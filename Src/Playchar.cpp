...
CPcProc::CPcProc(CGameMain* pGMain) : CBaseProc(pGMain)
{
	...
	// ＰＣ２　銃－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
	//m_pMeshArray.push_back( new CFbxMesh( m_pGMain->m_pFbxMeshCtrl, _T("Data/Char/PC2/pc2g.mesh")));    // ＰＣ２　銃
	...
	// ＰＣ２　剣－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
	//m_pMeshArray.push_back( new CFbxMesh( m_pGMain->m_pFbxMeshCtrl, _T("Data/Char/PC2/pc2s.mesh")));    // ＰＣ２　剣
	...
	// ＰＣ４　銃－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
	//m_pMeshArray.push_back( new CFbxMesh(m_pGMain->m_pFbxMeshCtrl, _T("Data/Char/PC4/pc4g.mesh")));    // ＰＣ４　銃
	...
	// ＰＣ４　剣－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
	//m_pMeshArray.push_back( new CFbxMesh(m_pGMain->m_pFbxMeshCtrl, _T("Data/Char/PC4/pc4s.mesh")));    // ＰＣ４　剣
	...
	// キャラクター　－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
	m_pMeshArray.push_back(new CFbxMesh(m_pGMain->m_pFbxMeshCtrl, _T("Data/Char/Character/character.mesh")));    // キャラクター
...
void	CPcObj::Update()
{
	...
	//vPcPos.y += 2.0f;
	//m_pGMain->m_pFont->Draw3D(vPcPos, m_pGMain->m_mView, m_pGMain->m_mProj, m_pGMain->m_vEyePt, _T("プレイキャラクター"), VECTOR2(0.5f, 0.2f), RGB(0, 0, 255), 1.0f, _T("HGP創英角ﾎﾟｯﾌﾟ体"));
...
void	CPcObj::UpdateNormalAttack()
{
	...
	if (pDI->CheckKey(KD_DAT, DIK_X) || pDI->CheckJoy(KD_DAT, DIJ_A) || pDI->CheckMouse(KD_DAT, DIM_LBUTTON)) // Xキー
	{
		...
		m_pGMain->m_pWeaponProc->m_pWeaponHoldProc->GetInfo(mHold, GetHoldItem(), mGun, vOffset); // ボーンマトリックスを指定して手に持つアイテムの情報を得る
		if (m_pGMain->m_pWeaponProc->m_pWeaponHoldProc->GetKind(GetHoldItem()) != 2)   // 銃を持っていないとき
		{
			vOffset.z = 0.8f;   // とりあえず前方に0.8
		}
		m_pGMain->m_pWeaponProc->m_pWeaponLaserProc->Start(mGun, vOffset, m_mWorld, PC); // レーザー発射
		m_pGMain->m_pSeLazer->Play(); // レーザー発射効果音
