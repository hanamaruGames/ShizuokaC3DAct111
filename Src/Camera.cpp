...
namespace {
	...
	const float RotationYSpeed = 60.0f; // １秒間の回転量
...
void CCamera::Update()
{
	...
	CPcProc* pPlayer = ObjectManager::FindGameObject<CPcProc>();
	if (pPlayer != nullptr) {
		MATRIX4X4 m = XMMatrixRotationY(pPlayer->Rotation().y * Deg2Rad);
		VECTOR3 pPosition = pPlayer->Position();
